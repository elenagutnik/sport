...
def jury_page(race_id):
    Competitors_list = db.session.query(Competitor, RunOrder).join(RunOrder, RunOrder.competitor_id==Competitor.id, isouter=True).filter(Competitor.race_id==race_id).order_by(RunOrder.group_id.asc()).all()
    treeView={}
    for item in Competitors_list:
        if item[1].group_id not in treeView.keys():
            treeView[item[1].group_id] = []
        treeView[item[1].group_id].append(item[0])

    runsList = db.session.query(RunInfo).filter(RunInfo.race_id == race_id).all()
