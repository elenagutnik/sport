...
#         order_by(asc(CourseDevice.order)).\
#         all()
#     courses_devices = db.session.query(CourseDevice, RunCourses).join(RunCourses, RunCourses.course_id == CourseDevice.course_id).filter(RunCourses.run_id == run_id).all()
#     for device in courses_devices:
#
#     if len(data) > 0:
#         for item in data:
#             try:
...
def TreeView(run_id):
    ...
    data = db.session.query(ResultApproved, ResultDetail, CourseDevice, CourseDeviceType, RunOrder). \
        ...
        order_by(asc(CourseDevice.order)). \
        all()
    courses_devices = db.session.query(CourseDevice, RunCourses).\
        join(RunCourses, RunCourses.course_id == CourseDevice.course_id).\
        filter(RunCourses.run_id == run_id).all()
    for device in courses_devices:
        if device[0].course_id not in tree_view.keys():
            tree_view[device[0].course_id] = {}
        if device[0].order not in tree_view[device[0].course_id].keys():
            tree_view[device[0].course_id][device[0].order] = {}
    if len(data) > 0:
        for item in data:
            try:
                if item[0].is_manual and item[0].status_id != 1:
...
# def ConvertRunResults(tree_view, manual_list, dql_list, run_id):
